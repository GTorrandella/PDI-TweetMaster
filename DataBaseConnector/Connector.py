...
def returnCampaignsInProgress():
    listaCampaignsBD = configTables.session.query(configTables.Campaign).filter(
        configTables.Campaign.startDate < datetime.now(),
        configTables.Campaign.finDate > datetime.now()).all()
    listaCampaigns = campaignsBDListToCampaignsList(listaCampaignsBD)
...
def returnCampaignsByEmail(user_email):
    ...
    campaigns = campaignsBDListToCampaignsList(campaignsBD)
    return campaigns

# Transforma lista de campaigns en formato de sqlalchemy a lista de campaigns
def campaignsBDListToCampaignsList (campaignsBDList):
    campaigns = []
...
def eliminarCampaignBDxID(idC):
    campaignespecifica = configTables.session.query(configTables.Campaign).get(
